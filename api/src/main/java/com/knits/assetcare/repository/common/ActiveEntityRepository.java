...
public interface ActiveEntityRepository<T extends AbstractActiveEntity> extends JpaRepository<T, Long>, JpaSpecificationExecutor<T> {
    ...
    default void delete(T entity) {
        ...
        Objects.requireNonNull(entity, "Entity from %s  cant be null".formatted(entityClassName));

        T entityToDelete = findById(entity.getId())
                .orElseThrow(() -> new RuntimeException(
                        "Entity from class %s and id %s not found".formatted(entityClassName, entity.getId()))
                );
